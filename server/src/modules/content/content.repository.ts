...
const toEntity = (row: QueryResultRow, config: CollectionConfig): Entity => {
    const entity: Entity = {
        id: row.id,
...
const valuesFromPayload = (payload: Record<string, unknown>, config: CollectionConfig) => {
    return Object.entries(config.fields)
        .filter(([apiKey]) => payload[apiKey] !== undefined)
        .map(([apiKey, dbKey]) => ({ apiKey, dbKey, value: payload[apiKey] }));
};
export const listItems = async (collection: string) => {
...
export const createItem = async (collection: string, payload: Record<string, unknown>) => {
    const config = getCollectionConfig(collection);
    ...
    const result = await query<QueryResultRow>(`INSERT INTO ${config.table} (${columns.join(', ')})
     VALUES (${params})
     RETURNING *`, values);
    return toEntity(result.rows[0] as QueryResultRow, config);
};
export const updateItem = async (collection: string, id: string, payload: Record<string, unknown>) => {
    const config = getCollectionConfig(collection);
    const entries = valuesFromPayload(payload, config);
    const assignments = entries.map((entry, index) => `${entry.dbKey} = $${index + 2}`);
    assignments.push('updated_at = NOW()');
    const result = await query<QueryResultRow>(`UPDATE ${config.table}
     SET ${assignments.join(', ')}
     WHERE id = $1
     RETURNING *`, [id, ...entries.map((entry) => entry.value)]);
    const row = result.rows[0] as QueryResultRow | undefined;
    return row ? toEntity(row, config) : null;
};
