import React from 'react';
import { useParams } from 'react-router-dom';
import { BookOpen, CheckCircle } from 'lucide-react';
import { DetailHero, VideoEmbed } from '@/src/components/details';
import { Container } from '@/src/components/ui';
import { useLanguage } from '@/src/context/LanguageContext';
import { getYouTubeEmbedUrl } from '@/src/lib/mediaPlayer';
import { clearSelectedLesson } from '@/src/modules/lessons/lessonsSlice';
import { fetchLessonItem, fetchLessons } from '@/src/modules/lessons/lessonsThunk';
import { useAppDispatch, useAppSelector } from '@/src/store/hooks';
const LessonDetail = () => {
    const { id } = useParams();
    const { language, t } = useLanguage();
    const dispatch = useAppDispatch();
    const lesson = useAppSelector((state) => state.lessons.selectedItem);
    React.useEffect(() => {
        dispatch(fetchLessons());
        if (id) {
            dispatch(fetchLessonItem(id));
        }
        return () => {
            dispatch(clearSelectedLesson());
        };
    }, [dispatch, id]);
    if (!lesson) {
        return (<div className="min-h-screen bg-slate-50 py-20">
        <Container>
          <p className="text-sm font-bold text-gray-400">Маалыматтар жүктөлүп жатат...</p>
        </Container>
      </div>);
    }
    const title = language === 'ky' ? lesson.title : (lesson.titleRu || lesson.title);
    const content = language === 'ky' ? lesson.content : (lesson.contentRu || lesson.content);
    const embedUrl = lesson.type === 'video' && lesson.videoUrl ? getYouTubeEmbedUrl(lesson.videoUrl) : '';
    const lessonTypeLabel = lesson.type === 'test'
        ? 'Тест'
        : lesson.type === 'video'
            ? language === 'ky' ? 'Видео сабак' : 'Видео урок'
            : language === 'ky' ? 'Текст сабак' : 'Текстовый урок';
    const videoFallback = language === 'ky'
        ? 'Видео URL туура эмес. YouTube шилтемесин кошуңуз.'
        : 'Видео URL указан неверно. Добавьте ссылку YouTube.';
    return (<div className="min-h-screen bg-white">
      <DetailHero
        backTo="/learning"
        backLabel={t('common.back')}
        eyebrow={lessonTypeLabel}
        title={title}
      />

      <Container className="py-12 md:py-20 px-6 md:px-12">
        {lesson.type === 'video' && (<VideoEmbed
          embedUrl={embedUrl}
          title={title}
          fallback={videoFallback}
          className="mb-10"
        />)}

        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
          <article className="lg:col-span-8 rounded-lg border border-gray-100 bg-white p-6 md:p-10 shadow-sm">
            <div className="mb-6 flex items-center gap-3 text-brand-primary">
              <BookOpen className="h-5 w-5"/>
              <h2 className="text-sm font-black uppercase tracking-widest">Материал</h2>
            </div>
            <p className="whitespace-pre-wrap text-base md:text-lg font-medium leading-relaxed text-gray-600">
              {content}
            </p>
          </article>

          {lesson.type === 'test' && (<aside className="lg:col-span-4 rounded-lg bg-slate-50 p-6 md:p-8 border border-gray-100">
              <h3 className="mb-6 text-sm font-black uppercase tracking-widest text-brand-primary">Практика</h3>
              {[
                'Кыргыз тили кайсы тилдер тобуна кирет?',
                'Мамлекеттик тилди өнүктүрүү эмне үчүн маанилүү?',
            ].map((question, index) => (<label key={question} className="mb-4 flex items-start gap-3 rounded-lg bg-white p-4 text-sm font-bold text-gray-600">
                  <CheckCircle className="mt-0.5 h-5 w-5 shrink-0 text-brand-primary"/>
                  <span>{index + 1}. {question}</span>
                </label>))}
            </aside>)}
        </div>
      </Container>
    </div>);
};
export default LessonDetail;
