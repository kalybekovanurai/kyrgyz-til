...
const MediaSurvey = () => {
    ...
    const surveys = allMedia.filter(m => m.type === 'survey' && (activeCategory === 'all' || m.category === activeCategory));
    React.useEffect(() => {
        dispatch(fetchMedia());
    }, [dispatch]);
    const categories = [
    ...
    return (<div className="flex flex-col w-full min-h-screen bg-slate-50">
        ...
        <div className="container mx-auto px-6 md:px-12">
          <div className="max-w-4xl">
            <div className="flex items-center gap-4 mb-6">
              ...
              <p className="text-[11px] font-black uppercase tracking-[0.3em] text-brand-primary">{t('media.survey.hero.title')}</p>
            </div>
            <h1 className="text-4xl md:text-6xl font-black text-brand-primary uppercase tracking-tighter leading-none">{t('nav.survey')}</h1>
          </div>
        </div>
      </section>
      ...
      <div className="container mx-auto px-6 md:px-12 py-12 md:py-20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {surveys.map((item, idx) => (<motion.div key={item.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.1 }} className="group bg-white rounded-3xl overflow-hidden border border-gray-100 shadow-sm hover:shadow-xl transition-all duration-500 flex flex-col">
              <div className="aspect-video relative overflow-hidden bg-brand-ink shrink-0">
                ...
                <Link to={`/media/survey/${item.id}`} className="cursor-pointer absolute inset-0 flex items-center justify-center" aria-label={t('common.view')}>
              ...
              <div className="p-8 flex-1 flex flex-col">
                ...
                <Link to={`/media/survey/${item.id}`} className="cursor-pointer block group/title">
