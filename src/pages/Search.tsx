...
const Search = () => {
    ...
    const filteredNews = news.filter(item => item.title.toLowerCase().includes(q.toLowerCase()) ||
        (item.titleRu && item.titleRu.toLowerCase().includes(q.toLowerCase())));
    const filteredMedia = media.filter(item => item.title.toLowerCase().includes(q.toLowerCase()) ||
        (item.titleRu && item.titleRu.toLowerCase().includes(q.toLowerCase())));
      ...
      <section className="bg-white border-b border-gray-100 py-16 md:py-24">
        <Container>
          <div className="max-w-4xl">
            <SectionHeading title={t('common.search_results')} subtitle={`${t('common.search_for')}: "${q}"`} className="mb-0"/>
          </div>
        </Container>
      </section>
      ...
      <Container className="py-12 md:py-20">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="space-y-6">
            {results.map((result, idx) => (<motion.div key={`${result.resultType}-${result.id}`} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.05 }}>
                <Link to={`/${result.resultType === 'news' ? 'news' : `media/${result.resultType === 'podcast' ? 'podcast' : 'survey'}`}/${result.id}`}>
                  <Card padding="lg" className="group hover:border-brand-primary transition-all overflow-hidden" hoverEffect>
                    <div className="flex flex-col md:flex-row gap-8">
                      {(result.image || result.thumbnail) ? (<div className="w-full md:w-48 h-32 rounded-xl overflow-hidden shrink-0">
            ...
            {results.length === 0 && (<Card padding="xl" className="text-center bg-white">
                ...
                <Link to="/" className="cursor-pointer text-brand-primary font-black uppercase text-[10px] tracking-widest hover:underline">
                  {t('common.go_home')}
                </Link>
              </Card>)}
          </div>
