...
export default function MediaPodcast() {
  ...
  const podcasts = allMedia.filter(m => 
    m.type === 'podcast' && (activeCategory === 'all' || m.category === activeCategory)
  );
          ...
          {podcasts.map((item, idx) => (
            ...
            >
              ...
              <div className="p-8 flex-1 flex flex-col">
                ...
                <Link to={`/media/podcast/${item.id}`} className="block group/title">
                ...
                <div className="flex items-center justify-between pt-6 border-t border-gray-50">
                  <Link to={`/media/podcast/${item.id}`} className="text-[10px] font-black uppercase tracking-widest text-brand-primary flex items-center gap-2 group-hover:translate-x-1 transition-transform">
