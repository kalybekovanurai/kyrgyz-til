...
export default function Media() {
  ...
  const filteredMedia = allMedia.filter(m => 
    activeTopic === 'all' || m.category === activeTopic
  );
      ...
      <section className="bg-white border-b border-gray-100 py-16 md:py-20">
        <Container>
          <div className="max-w-4xl">
            <SectionHeading 
              title={t('nav.media')} 
              subtitle={t('media.hero.subtitle')}
              className="mb-8"
            />
          ...
          <div className="mt-12">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
              {filteredMedia.map((item) => (
                <Link key={item.id} to={`/media/${item.type}/${item.id}`} className="block h-full">
              ...
              {filteredMedia.length === 0 && !loading && (
                <div className="col-span-full py-12 text-center text-gray-400 font-bold uppercase text-xs tracking-widest bg-white rounded-3xl border-2 border-dashed border-gray-100">
