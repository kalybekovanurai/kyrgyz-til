    ...
    return (<div className="flex flex-col w-full min-h-screen bg-white">
      <section className="py-20 bg-slate-50 border-b border-gray-100">
        <div className="container mx-auto px-6 md:px-12">
          <Link to="/news" className="cursor-pointer inline-flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-gray-400 hover:text-brand-primary transition-colors mb-12">
            ...
            <div className="sticky top-24 space-y-8">
              <div className="bg-slate-50 p-10 rounded-[3rem] border border-gray-100">
                ...
                <div className="space-y-8">
                  {news.filter(n => n.id !== newsItem.id).slice(0, 3).map((item) => (<Link key={item.id} to={`/news/${item.id}`} className="cursor-pointer block group">
