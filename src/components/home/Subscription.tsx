          ...
          <div className="relative z-10 grid grid-cols-1 lg:grid-cols-2 gap-12 md:gap-16 items-center">
            <div>
              <div className="flex items-center gap-4 mb-6">
                ...
                <h2 className="text-xl md:text-3xl font-black text-brand-primary uppercase tracking-tighter italic leading-tight">{language === 'ky' ? subscription.titleKy || t('sub.title') : subscription.titleRu || subscription.titleKy || t('sub.title')}</h2>
              </div>
              <p className="text-sm font-medium text-gray-500 leading-relaxed mb-10">
                {language === 'ky' ? subscription.descriptionKy || t('sub.desc') : subscription.descriptionRu || subscription.descriptionKy || t('sub.desc')}
              </p>
              <div className="flex items-center gap-4 p-4 bg-white rounded-2xl border border-gray-200 shadow-inner">
                 ...
                 <span className="text-[10px] md:text-[11px] font-black text-gray-400 tracking-widest uppercase">{language === 'ky' ? subscription.safeTextKy || t('sub.safe') : subscription.safeTextRu || subscription.safeTextKy || t('sub.safe')}</span>
              </div>
            </div>
            <SubscriptionForm />
          </div>
