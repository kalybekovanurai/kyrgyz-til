         ...
         <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 md:gap-12 items-center">
            <div>
              ...
              <Button icon={ArrowRight}>
                {t('learning.methods.download')}
              </Button>
            </div>
            <div className="grid grid-cols-2 gap-4">
               {[1, 2, 3, 4].map(i => (
                 <div key={i} className="aspect-square bg-slate-50 rounded-2xl md:rounded-3xl border border-gray-100 flex flex-col items-center justify-center gap-2 p-4 md:p-6 hover:bg-white transition-colors cursor-pointer group">
                   <FileText className="w-6 h-6 md:w-8 md:h-8 text-brand-primary opacity-20 group-hover:opacity-100 transition-all" />
                   <span className="text-[9px] md:text-[10px] font-black uppercase tracking-widest text-gray-400">{t('learning.methods.item')}{i}</span>
                 </div>
               ))}
            </div>
         </div>
